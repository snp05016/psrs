...
void merge(vector<vector<int>>poVector ){ //poVector will be the vector of partially ordered vectors
    vector<int> pointer_tracker(poVector.size() , 0 ); // this tracks the index of the element that we are at.
    int last_updated_sorted_vec;
    int index = 0 ;
    std::priority_queue< std::pair<int, int>, std::vector<std::pair<int, int>>, std::greater<std::pair<int, int>> > merge_pq; // initialize a minheap using a pq
...
//        cout << "the poped element is " << smallest_element.first  << "and the index popped from is" << smallest_element.second << endl;
        ...
        merge_pq.pop();
        if (pointer_tracker[last_updated_sorted_vec] != (poVector[last_updated_sorted_vec].size())){
            merge_pq.push({(poVector[last_updated_sorted_vec][pointer_tracker[last_updated_sorted_vec]]), last_updated_sorted_vec});
        }
    }
    for ( const int elem : sorted_vec ){
        cout << elem << endl;
    }
}
