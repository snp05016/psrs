...
vector<int> PSRS(vector<int> &inputVector, int n,
  ...
  for (int processor = 0; processor < p; processor++) {
    for (int bucket = 0; bucket < p; bucket++) {
      int seg_start = subsize[processor * p + bucket];
      int seg_end = subsize[processor * p + bucket + 1];
      if (seg_start < seg_end && seg_end <= (int)inputVector.size()) {
        buckets[bucket].push_back(vector<int>(inputVector.begin() + seg_start,
                                              inputVector.begin() + seg_end));
      }
    }
  }
  gettimeofday(&end, NULL);
